...
(iii) Sequence demultiplexing and quality assessment (Steps 7-10): High-throughput sequence processing, barcode identification with error correction, and quality control metric generation

(iv) Data visualization and interpretation (Step 11): Interactive dashboard exploration
...
\subsubsection{Phase 2: Chemical library enumeration and analysis • TIMING 10 min - 2 h}
...
\paragraph{Step 4 \textbar{} Enumerate library compounds from building blocks}

Generate all possible chemical structures by combining building blocks according to the defined reaction schemes:


\begin{lstlisting}
delt-hit library enumerate --config_path=/path/to/config.yaml --graph_only=True  # produce reaction graph only for inspection
delt-hit library enumerate --config_path=/path/to/config.yaml
\end{lstlisting}
Those commands:

\begin{itemize}
  \item Construct a reaction graph from your building blocks and reactions
  \item Enumerates all valid compound combinations
  \item Validates chemical structures
  \item Saves the complete library catalog
\end{itemize}
Expected outputs:

\begin{itemize}
  \item library.parquet: Complete compound catalog with SMILES and barcode mappings
...
\paragraph{Step 5 \textbar{} Calculate molecular properties and descriptors}
...
\paragraph{Step 6 \textbar{} Generate molecular representations for machine learning}
...
\subsubsection{Phase 3: Sequence processing and demultiplexing • TIMING 30 min - 4 h}
This phase processes raw sequencing reads to identify and count each library member in your selection experiments.


\paragraph{Step 7 \textbar{} Configure demultiplexing parameters}
...
\paragraph{Step 8 \textbar{} Prepare and execute sequence demultiplexing}
...
\paragraph{Step 9 \textbar{} Process demultiplexing results and generate count tables}
...
\paragraph{Step 10 \textbar{} Generate quality control reports}
...
\subsubsection{Phase 4: Data visualization and interpretation • TIMING 15-60 min}
The interactive dashboard provides real-time exploration of raw count data and experimental metadata.


\paragraph{Step 11 \textbar{} Launch interactive analysis dashboard}
