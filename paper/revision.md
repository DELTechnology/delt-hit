...
- The provided DOCX appears to be a Procedure-format excerpt rather than a complete example paper. In the extracted text, it begins at `Procedure` and shows the expected formatting of phase headers, numbered steps, `CRITICAL STEP` callouts, and timing annotations, but it does not expose the full front-matter and back-matter structure.
