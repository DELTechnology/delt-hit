...
DEL architectures include single-display and dual-display formats, depending on whether the pharmacophore is presented on one DNA-linked fragment or on two pairing oligonucleotide strands\cite{lessing_advancingsmallmoleculedrug_2023}. Standard libraries are often encoded on double-stranded DNA headpieces, but single-display and Y-shaped encoding schemes are also used\cite{plais_universalencodingnext_2022}. Regardless of the encoding format, selection and PCR amplification ultimately produce double-stranded amplicons for sequencing and downstream analysis\cite{decurtins_automatedscreeningsmall_2016}.
